 ...
 * 
 * The plugin drastically enhances the HTML file input to preview multiple upload on the client before
 * upload. In addition it provides the ability to preview content of images, text, videos, audio, html, 
 * flash and other objects. It also offers the ability to upload and delete upload using AJAX, and add
 * upload in batches (i.e. preview, append, or remove before upload).
 * 
    ...
    FileInput.prototype = {
        ...
        noFilesError: function (params) {
            var self = this, label = self.minFileCount > 1 ? self.filePlural : self.fileSingle,
                msg = self.msgFilesTooLess.replace('{n}', self.minFileCount).replace('{upload}', label),
        ...
        getMsgSelected: function (n) {
            ...
            return self.msgSelected.replace('{n}', n).replace('{upload}', strFiles);
        },
        ...
        readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if ($preview.length > 0 && FileReader !== undefined) {
                    $status.html(msgLoading.replace('{index}', i + 1).replace('{upload}', numFiles));
                    ...
                    reader.onloadend = function () {
                        msg = msgProgress
                            .replace('{index}', i + 1).replace('{upload}', numFiles)
                    ...
                    reader.onprogress = function (data) {
                        if (data.lengthComputable) {
                            ...
                            msg = msgProgress.replace('{index}', i + 1).replace('{upload}', numFiles)
...
//FileInput plugin definition
    ...
    $.fn.fileinputLocales.en = {
        ...
        removeLabel: 'Remove',
        removeTitle: 'Clear selected upload',
        cancelLabel: 'Cancel',
        cancelTitle: 'Abort ongoing upload',
        uploadLabel: 'Upload',
        uploadTitle: 'Upload selected upload',
        msgZoomTitle: 'View details',
        msgZoomModalHeading: 'Detailed Preview',
        msgSizeTooLarge: 'File "{name}" (<b>{size} KB</b>) exceeds maximum allowed upload size of <b>{maxSize} KB</b>.',
        msgFilesTooLess: 'You must select at least <b>{n}</b> {upload} to upload.',
        msgFilesTooMany: 'Number of upload selected for upload <b>({n})</b> exceeds maximum allowed limit of <b>{m}</b>.',
        msgFileNotFound: 'File "{name}" not found!',
        msgFileSecured: 'Security restrictions prevent reading the file "{name}".',
        msgFileNotReadable: 'File "{name}" is not readable.',
        msgFilePreviewAborted: 'File preview aborted for "{name}".',
        msgFilePreviewError: 'An error occurred while reading the file "{name}".',
        msgInvalidFileType: 'Invalid type for file "{name}". Only "{types}" upload are supported.',
        msgInvalidFileExtension: 'Invalid extension for file "{name}". Only "{extensions}" upload are supported.',
        msgUploadAborted: 'The file upload was aborted',
        msgValidationError: 'File Upload Error',
        msgLoading: 'Loading file {index} of {upload} &hellip;',
        msgProgress: 'Loading file {index} of {upload} - {name} - {percent}% completed.',
        msgSelected: '{n} {upload} selected',
        msgFoldersNotAllowed: 'Drag & drop upload only! {n} folder(s) dropped were skipped.',
        ...
        dropZoneTitle: 'Drag & drop upload here &hellip;'
    };
