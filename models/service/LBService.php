 ...
 */
require_once __DIR__.'/../entity/LBDomain.php';
...
class LBService
{
    ...
    {
        $lbDomain = new LBDomain();
        $lbTable = $lbDomain->cdashTable;
        if($lbDomain->isGetFromFile) {
            array_pop($lbTable);
        }
        ...
        $specialVars = array("LBDAT", "LBTIM", "LBDTC",); $lbFlag = false;
        ...
        foreach ($this->titleArr as $i => $val) {
            ...
            else if(!key_exists($cdashVal, $lbTable) && !key_exists($cdashVal, $commonTable)) {
            ...
            else if(!key_exists($cdashVal, $commonTable) && key_exists($cdashVal, $lbTable)) {
                if($lbTable[$cdashVal] != "") {
                    if(!in_array($cdashVal, $specialVars)) {
                        array_push($sdtmTable, $lbTable[$cdashVal]);
                    }
                    else if(in_array($cdashVal, $specialVars) && $lbFlag == false) {
                        $lbFlag = true;
                        array_push($sdtmTable, $lbTable[$cdashVal]);
                    }
                }
        ...
        $userService->RealSaveFunction($sdtmStr, $newDataTable,
            $nonMapTitleStr, $deleteDataTable, $lbDomain->domainName);
    }
    ...
    public function TimeDataOperation($titleArr,$dataArr)
    {
        ...
        $lbDtcLoc = array_search("LBDTC",$this->titleArr);
        $lbDatLoc = array_search("LBDAT",$this->titleArr);
        $lbTimLoc= array_search("LBTIM",$this->titleArr);
    ...
    public function SaveFile($sdtmStr, $newDataTable)
    {
        session_start();
        $op = "\r\n";
        $fileName = "SDTM-LB(Submit Version).csv";
    ...
    {
        $op = "\r\n";
        $fileName = "SDTM-LB(with deleted data).csv";
        $path = "../../files1/".$fileName;
